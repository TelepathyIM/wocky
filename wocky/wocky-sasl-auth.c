...
wocky_sasl_auth_error_quark (void) {
  ...
  if (!quark)
    quark = g_quark_from_static_string ("salut_wocky_auth_failed");
...
wocky_sasl_auth_class_init (WockySaslAuthClass *wocky_sasl_auth_class)
{
  ...
  g_type_class_add_private (wocky_sasl_auth_class, sizeof (WockySaslAuthPrivate));
...
wocky_sasl_auth_mechanisms_to_list(WockyXmppNode *mechanisms)  {
  ...
  wocky_xmpp_node_each_child(mechanisms, 
                              each_mechanism,
                              &result);
  return result;
}
...
static void
digest_md5_stanza_received(WockyXmppConnection *connection, 
    WockyXmppStanza *stanza,
    WockySaslAuth *sasl) 
{
  int i;
...
#undef HANDLE
  ...
  if (strcmp(wocky_xmpp_node_get_ns(stanza->node), WOCKY_XMPP_NS_SASL_AUTH)) {
    auth_failed(sasl, WOCKY_SASL_AUTH_ERROR_INVALID_REPLY, 
...
static gboolean
wocky_sasl_auth_start_digest_md5(WockySaslAuth *sasl, GError **error) {
  WockyXmppStanza *stanza;
  WockySaslAuthPrivate *priv = WOCKY_SASL_AUTH_GET_PRIVATE(sasl);
  GError *send_error = NULL;
  gboolean ret = TRUE;
...
wocky_sasl_auth_authenticate(WockySaslAuth *sasl,
  ...
  mech_node = wocky_xmpp_node_get_child_ns(features->node,  
                                             "mechanisms",
                                             WOCKY_XMPP_NS_SASL_AUTH);
  ...
  if (wocky_sasl_auth_has_mechanism(mechanisms, "DIGEST-MD5")) {
    ...
    ret = wocky_sasl_auth_start_digest_md5(sasl, error);
  } else {
    g_set_error(error, 
        WOCKY_SASL_AUTH_ERROR,
