 ...
 *
 * This is a hacked implementation which fakes a particular sequence
 * of partial and complete SSL records: given a block of data X
 * in a write, it will transmit:
 *
 * a short complete SSL record encrypting TEST_SSL_RECORD_MARKER
 * half an SSL record (or records) corresponding to your data
 *
 * in a single write attempt, followed by:
 *
 * the second half of your SSL record
 * a short complete RECORD encrypting TEST_SSL_RECORD_MARKER
 */
...
typedef GIOStreamClass TestTLSConnectionClass;
typedef GObjectClass TestTLSSessionClass;
typedef GInputStreamClass TestTLSInputStreamClass;
typedef GOutputStreamClass TestTLSOutputStreamClass;

struct OPAQUE_TYPE__TestTLSSession
{
  GObject parent;
...
struct OPAQUE_TYPE__TestTLSConnection
{
  GIOStream parent;

  TestTLSSession *session;
  TestTLSInputStream *input;
  TestTLSOutputStream *output;
};
...
static GType test_tls_input_stream_get_type (void);
static GType test_tls_output_stream_get_type (void);
G_DEFINE_TYPE (TestTLSConnection, test_tls_connection, G_TYPE_IO_STREAM);
G_DEFINE_TYPE (TestTLSSession, test_tls_session, G_TYPE_OBJECT);
G_DEFINE_TYPE (TestTLSInputStream, test_tls_input_stream, G_TYPE_INPUT_STREAM);
G_DEFINE_TYPE (TestTLSOutputStream, test_tls_output_stream, G_TYPE_OUTPUT_STREAM);
#define TEST_TYPE_TLS_INPUT_STREAM (test_tls_input_stream_get_type ())
#define TEST_TYPE_TLS_OUTPUT_STREAM (test_tls_output_stream_get_type ())
#define TEST_TLS_INPUT_STREAM(inst) (G_TYPE_CHECK_INSTANCE_CAST ((inst), \
                                                                 TEST_TYPE_TLS_INPUT_STREAM, \
                                                                 TestTLSInputStream))
#define TEST_TLS_OUTPUT_STREAM(inst) (G_TYPE_CHECK_INSTANCE_CAST ((inst), \
                                                                  TEST_TYPE_TLS_OUTPUT_STREAM, \
                                                                  TestTLSOutputStream))

GQuark
test_tls_cert_error_quark (void)
{
  static GQuark quark = 0;

  if (quark == 0)
    quark = g_quark_from_static_string ("test-tls-cert-error");

  return quark;
}

GQuark
test_tls_error_quark (void)
{
  static GQuark quark = 0;

  if (quark == 0)
    quark = g_quark_from_static_string ("test-tls-error");
...
test_tls_job_make_result (TestTLSJob *job,
                          gssize   result)
{
  ...
  if (job->error != NULL)
    {
      S_DBG ("setting error from job '%s'", job->error->message);
...
static void
test_tls_job_result_gssize (TestTLSJob *job,
                            gssize   result)
{
  GSimpleAsyncResult *simple;

  if ((simple = test_tls_job_make_result (job, result)))
    {
      if (result >= 0)
...
static void
test_tls_job_result_boolean (TestTLSJob *job,
                             gint     result)
{
  GSimpleAsyncResult *simple;
  ...
  if (job->error == NULL)
    switch (result)
      ...
      case SSL_ERROR_WANT_WRITE:
        break;
      default:
        S_DBG ("setting up handshake error");
        job->error = g_error_new (TEST_TLS_ERROR, result, "Handshake Error");
      }
...
/* ************************************************************************* */
static void
test_tls_session_try_operation (TestTLSSession   *session,
                                TestTLSOperation  operation);

static void
test_tls_session_write_ready (GObject      *object,
                              GAsyncResult *result,
                              gpointer      user_data);
static void
test_tls_session_read_ready (GObject      *object,
                             GAsyncResult *result,
                             gpointer      user_data);
 ...
 * clearly happen */
static void
handshake_write (TestTLSSession *session)
{
  gchar *wbuf;
  TestTLSJob *handshake = &(session->job.handshake.job);
  ...
  g_output_stream_write_async (output, wbuf, wsize, prio, cancel,
                               test_tls_session_write_ready, session);
}

static void
handshake_read (TestTLSSession *session)
{
  GInputStream *input = g_io_stream_get_input_stream (session->stream);
  TestTLSJob *handshake = (TestTLSJob *) &session->job.handshake.job;
  ...
  g_input_stream_read_async (input,
                             ...
                             test_tls_session_read_ready,
                             session);
}

static int
ssl_handshake (TestTLSSession *session)
{
  gint result = 1;
  ...
  if (!done)
    {
      ...
      S_DBG ("%s - result: %d; error: %ld", method, result, errnum);
      S_DBG ("%s         : %s", method, errstr);
    }
  ...
  if (done)
    {
      ...
      if (!want_write)
        {
          S_DBG ("Handshake completed");
          errnum = session->job.handshake.state = SSL_ERROR_NONE;
        }
      else
        {
          S_DBG ("Handshake completed (IO incomplete)");
    ...
    {
      S_DBG ("Handshake state: %ld", errnum);
   ...
   * invokes test_tls_session_write_ready which will in turn call        *
   * test_tls_session_try_operation which will re-enter handshake        *
   * and then proceed to fall back through to this block of code          */

  if (!fatal)
    {
      S_DBG ("want write: %d; want read: %d;", want_write, want_read);
      if (want_write)
        handshake_write (session);
      else if (want_read)
        handshake_read (session);
      else
        test_tls_session_try_operation (session, TEST_TLS_OP_HANDSHAKE);
    }
  else
    {
      S_DBG ("Handshake failed: [%d:%ld] %s", result, errnum, errstr);
      if (session->job.handshake.job.error)
        {
          g_error_free (session->job.handshake.job.error);
          session->job.handshake.job.error = NULL;
        }
      g_set_error (&(session->job.handshake.job.error), TEST_TLS_ERROR, result,
...
static void
ssl_fill (TestTLSSession *session)
{
  GInputStream *input = g_io_stream_get_input_stream (session->stream);
  ...
  if (SSL_pending (session->ssl) > 0)
    {
      S_DBG ("SSL record already available");
      test_tls_session_try_operation (session, TEST_TLS_OP_READ);
      return;
    }
  g_input_stream_read_async (input, rbuf, MAX_SSLV3_BLOCK_SIZE, prio, cancel,
                             test_tls_session_read_ready, session);
}

static gboolean
ssl_flush_stage_2 (gpointer data)
{
  long wsize;
  long offset;
  gchar *wbuf;
  TestTLSSession *session = data;
  gint prio = session->job.read.io_priority;
  GOutputStream *output = g_io_stream_get_output_stream (session->stream);
  GCancellable *cancel = session->job.read.cancellable;
...
static void
ssl_flush (TestTLSSession *session)
{
  long wsize;
  gchar *wbuf;
  GOutputStream *output = g_io_stream_get_output_stream (session->stream);
  ...
  if (wsize > 0)
    {
      long offset = wsize / 2;
      /* split the write in two so the middle record gets split     */
      /* if this is a user-oroginated write, we will have prepended *
       * and appended a short record to the real one by this point  */
      session->extra_write =
        g_output_stream_write (output, wbuf, offset, cancel, NULL);
      g_output_stream_flush (output, cancel, NULL);
      S_DBG ("%ld bytes partial write", session->extra_write);
      g_idle_add (ssl_flush_stage_2, g_object_ref (session));
    }
}
                                ...
                                TestTLSOperation  operation)
{
  ...
  if (handshake->active || operation == TEST_TLS_OP_HANDSHAKE)
    {
      gint result = session->job.handshake.state;
      S_DBG ("async job handshake");

      if (tls_debug_level >= DEBUG_HANDSHAKE_LEVEL)
        S_DBG ("async job handshake: %d", result);

      switch (result)
        {
        case SSL_ERROR_WANT_READ:
        case SSL_ERROR_WANT_WRITE:
          S_DBG ("Handshake incomplete...");
          ssl_handshake (session);
          break;
        case SSL_ERROR_NONE:
        default:
          S_DBG ("Handshake complete: %d", result);
          test_tls_job_result_boolean (handshake, result);
        }
    }
  else if (operation == TEST_TLS_OP_READ)
    {
      gssize result = 0;
      gulong pending = 0;
      gsize wanted = 0;
      ...
      S_DBG ("read %ld clearbytes (from %ld cipherbytes)", result, pending);
      ...
      if ((session->job.read.error == NULL) && (result <= 0))
        {
          ...
          switch (error)
            {
            case SSL_ERROR_WANT_READ:
              S_DBG ("Incomplete SSL record, read again");
              ssl_fill (session);
              return;
            case SSL_ERROR_WANT_WRITE:
            ...
            default:
              ...
              if(session->job.read.error == NULL)
                session->job.read.error =
                  g_error_new (TEST_TLS_ERROR, error,
                               "OpenSSL read: protocol error %d", error);
            }
        }

      test_tls_job_result_gssize (&session->job.read, result);
    }
    ...
    { /* we have no useful way of mapping SSL cipherbytes to raw *
      ...
      if (tls_debug_level >= DEBUG_ASYNC_DETAIL_LEVEL)
        S_DBG ("async job OP_WRITE");

      g_assert (operation == TEST_TLS_OP_WRITE);
      S_DBG ("wrote %ld clearbytes", result);
      test_tls_job_result_gssize (&session->job.write, result);
    }
}

static void
test_tls_job_start (TestTLSJob             *job,
...
TestTLSConnection *
test_tls_session_handshake (TestTLSSession   *session,
                            GCancellable  *cancellable,
                            GError       **error)
{
  gint result = -1;
  ...
  while (go)
    {
      ...
      if (!done)
        {
          ...
          if (errnum != SSL_ERROR_NONE &&
            ...
            {
              errstr = error_to_string (errnum);
              S_DBG ("SSL handshake error: [%d:%d] %s", result, errnum, errstr);
            }
        }
      ...
      if (want_write)
        {
          ...
          long wsize = BIO_get_mem_data (session->wbio, &wbuf);
          gssize sent = 0;
          S_DBG ("sending %ld cipherbytes", wsize);
          if (wsize > 0)
            sent = g_output_stream_write (out, wbuf, wsize, NULL, error);
          S_DBG ("sent %ld cipherbytes", sent);
      ...
      if (want_read)
        {
          ...
          S_DBG ("read %ld cipherbytes", bytes);
      ...
      switch (errnum)
        ...
        case SSL_ERROR_WANT_READ:
          break;
        case SSL_ERROR_NONE:
          S_DBG ("handshake complete, all IO done");
          go = FALSE;
          break;
        default:
          S_DBG ("SSL handshake error: [%d:%d] %s", result, errnum, errstr);
          *error =
            g_error_new (TEST_TLS_ERROR, errnum, "Handshake: %s", errstr);
          go = FALSE;
        }
    }

  if (done)
    return g_object_new (TEST_TYPE_TLS_CONNECTION, "session", session, NULL);
...
static void
add_ca_or_crl (TestTLSSession *session,
               const gchar *path,
               const gchar *label)
{
  gboolean ok = FALSE;
  ...
  if (stat (path, &target) != 0)
    {
      S_DBG ("%s file or path '%s' not accessible", label, path);
      return;
    }

  if (S_ISDIR (target.st_mode))
    {
      S_DBG ("Loading %s directory", label);
      ok = SSL_CTX_load_verify_locations (session->ctx, NULL, path);
    }

  if (S_ISREG (target.st_mode))
    {
      S_DBG ("Loading %s file", label);
  ...
  if (!ok)
    {
      ...
      for (f = e = ERR_get_error (); e != 0; e = ERR_get_error ())
        f = e;
      S_DBG ("%s '%s' failed: %s", label, path, ERR_error_string (f, NULL));
    }
  else
    S_DBG ("%s '%s' loaded", label, path);
}

void
test_tls_session_add_ca (TestTLSSession *session,
                         const gchar *path)
{
...
void
test_tls_session_add_crl (TestTLSSession *session,
                          const gchar *path)
{
...
{
  S_DBG ();
  test_tls_job_start (&session->job.handshake.job, session,
                      io_priority, cancellable, callback, user_data,
                      test_tls_session_handshake_async);
  ssl_handshake (session);
}
                                   ...
                                   GError       **error)
{
  ...
  g_return_val_if_fail (test_tls_session_handshake_async ==
...
int
test_tls_session_verify_peer (TestTLSSession    *session,
                              ...
                              TestTLSCertStatus *status)
{
  ...
  X509 *cert;
  gboolean lenient = (flags == TEST_TLS_VERIFY_LENIENT);

  S_DBG ();
  g_assert (status != NULL);
  *status = TEST_TLS_CERT_OK;

  switch (flags)
    {
    case TEST_TLS_VERIFY_STRICT:
      check_level = "TEST_TLS_VERIFY_STRICT";
      break;
    case TEST_TLS_VERIFY_NORMAL:
      check_level = "TEST_TLS_VERIFY_NORMAL";
      break;
    case TEST_TLS_VERIFY_LENIENT:
      check_level = "TEST_TLS_VERIFY_LENIENT";
      break;
    default:
      check_level = "*custom setting*";
    }

  S_DBG ("setting ssl verify flags level to: %s", check_level);
  cert = SSL_get_peer_certificate (session->ssl);
  rval = SSL_get_verify_result (session->ssl);
  S_DBG ("X509 cert: %p; verified: %d", cert, rval);
  ...
  if (cert == NULL && rval == X509_V_OK)
    {
      S_DBG ("Anonymous SSL handshake");
      rval = X509_V_ERR_CERT_UNTRUSTED;
    }

  if (rval != X509_V_OK)
    {
      S_DBG ("cert verification error: %d", rval);
      switch (rval)
        {
        ...
        case X509_V_ERR_SUBJECT_ISSUER_MISMATCH:
          *status = TEST_TLS_CERT_SIGNER_UNKNOWN;
          break;
        ...
        case X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT:
          *status = TEST_TLS_CERT_INVALID;
          break;
        case X509_V_ERR_CERT_REVOKED:
          *status = TEST_TLS_CERT_REVOKED;
          break;
        case X509_V_ERR_ERROR_IN_CERT_NOT_BEFORE_FIELD:
        case X509_V_ERR_CERT_NOT_YET_VALID:
          *status = TEST_TLS_CERT_NOT_ACTIVE;
          break;
        case X509_V_ERR_ERROR_IN_CERT_NOT_AFTER_FIELD:
        case X509_V_ERR_CERT_HAS_EXPIRED:
          *status = TEST_TLS_CERT_EXPIRED;
          break;
        case X509_V_ERR_OUT_OF_MEM:
          *status = TEST_TLS_CERT_INTERNAL_ERROR;
          break;
        ...
        case X509_V_ERR_KEYUSAGE_NO_CERTSIGN:
          *status = TEST_TLS_CERT_SIGNER_UNAUTHORISED;
          break;
        case X509_V_ERR_PATH_LENGTH_EXCEEDED:
          *status = TEST_TLS_CERT_MAYBE_DOS;
          break;
        default:
          *status = TEST_TLS_CERT_UNKNOWN_ERROR;
        }
      ...
      if (lenient)
        switch (*status)
          {
          case TEST_TLS_CERT_INTERNAL_ERROR:
          case TEST_TLS_CERT_REVOKED:
          case TEST_TLS_CERT_MAYBE_DOS:
            break;
          default:
            rval = X509_V_OK;
            *status = TEST_TLS_CERT_OK;
  ...
  S_DBG ("peer_name_ok: %d", peer_name_ok );
  ...
  if (!peer_name_ok)
    {
      *status = TEST_TLS_CERT_NAME_MISMATCH;
...
static gssize
test_tls_input_stream_read (GInputStream  *stream,
                            void          *buffer,
                            gsize          count,
                            GCancellable  *cancellable,
                            GError       **error)
{
  /* TestTLSSession *session = TEST_TLS_INPUT_STREAM (stream)->session; */

  S_DBG ("sync read - not implmented");
...
static void
test_tls_input_stream_read_async (GInputStream        *stream,
                                  void                *buffer,
                                  gsize                count,
                                  gint                 io_priority,
                                  GCancellable        *cancellable,
                                  ...
                                  gpointer             user_data)
{
  ...
  test_tls_job_start (&session->job.read, stream,
                      io_priority, cancellable, callback, user_data,
                      test_tls_input_stream_read_async);
...
static gssize
test_tls_input_stream_read_finish (GInputStream  *stream,
                                   GAsyncResult  *result,
                                   GError       **error)
{
  GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);
  ...
  g_return_val_if_fail (test_tls_input_stream_read_async ==
...
static gssize
test_tls_output_stream_write (GOutputStream  *stream,
                              const void     *buffer,
                              gsize           count,
                              GCancellable   *cancellable,
                              GError        **error)
{
  /* TestTLSSession *session = TEST_TLS_OUTPUT_STREAM (stream)->session; */

  S_DBG ("sync write - not implemented");
...
static void
test_tls_output_stream_write_async (GOutputStream       *stream,
                                    const void          *buffer,
                                    gsize                count,
                                    gint                 io_priority,
                                    GCancellable        *cancellable,
                                    ...
                                    gpointer             user_data)
{
  ...
  S_DBG ("%ld clearbytes to send", count);
  test_tls_job_start (&session->job.write, stream,
                      io_priority, cancellable, callback, user_data,
                      test_tls_output_stream_write_async);

  c    += SSL_write (session->ssl, TEST_SSL_RECORD_MARKER,
                     TEST_SSL_RECORD_MARKER_LEN);
  wsize = BIO_get_mem_data (session->wbio, &wbuf);
  S_DBG ("%d/%ld clear/cipher bytes to send", code + c, wsize);
  ...
  session->job.write.count = count + c;
  ...
  if (code < 0)
    {
      ...
      switch (error)
        {
        case SSL_ERROR_WANT_WRITE:
          S_DBG ("Incomplete SSL write to BIO (theoretically impossible)");
          ssl_flush (session);
          return;
        case SSL_ERROR_WANT_READ:
          g_warning ("write caused read: unsupported TLS re-negotiation?");
        default:
          S_DBG ("SSL write failed, setting error %d", error);
          /* if we haven't already generated an error, set one here: */
          if(session->job.write.error == NULL)
            session->job.write.error =
              g_error_new (TEST_TLS_ERROR, error,
                           "OpenSSL write: protocol error %d", error);
          test_tls_session_try_operation (session, TEST_TLS_OP_WRITE);
          return;
        }
    }
  ssl_flush (session);
...
static gssize
test_tls_output_stream_write_finish (GOutputStream   *stream,
                                     GAsyncResult   *result,
                                     GError        **error)
{
  GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);
  ...
  g_return_val_if_fail (test_tls_output_stream_write_async ==
                        g_simple_async_result_get_source_tag (simple), -1);
...
static void
test_tls_output_stream_init (TestTLSOutputStream *stream)
{
}

static void
test_tls_input_stream_init (TestTLSInputStream *stream)
{
}

static void
test_tls_output_stream_set_property (GObject *object, guint prop_id,
                                     const GValue *value, GParamSpec *pspec)
{
  TestTLSOutputStream *stream = TEST_TLS_OUTPUT_STREAM (object);
...
static void
test_tls_output_stream_constructed (GObject *object)
{
  TestTLSOutputStream *stream = TEST_TLS_OUTPUT_STREAM (object);
...
static void
test_tls_output_stream_finalize (GObject *object)
{
  TestTLSOutputStream *stream = TEST_TLS_OUTPUT_STREAM (object);

  g_object_unref (stream->session);

  G_OBJECT_CLASS (test_tls_output_stream_parent_class)
    ->finalize (object);
}

static void
test_tls_output_stream_class_init (GOutputStreamClass *class)
{
  GObjectClass *obj_class = G_OBJECT_CLASS (class);

  class->write_fn = test_tls_output_stream_write;
  class->write_async = test_tls_output_stream_write_async;
  class->write_finish = test_tls_output_stream_write_finish;
  obj_class->set_property = test_tls_output_stream_set_property;
  obj_class->constructed = test_tls_output_stream_constructed;
  obj_class->finalize = test_tls_output_stream_finalize;
  ...
  g_object_class_install_property (obj_class, PROP_O_SESSION,
                                   g_param_spec_object ("session", "TLS session",
                                                        ...
                                                        TEST_TYPE_TLS_SESSION, G_PARAM_WRITABLE |
                                                        G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME |
                                                        G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}

static void
test_tls_input_stream_set_property (GObject *object, guint prop_id,
                                    const GValue *value, GParamSpec *pspec)
{
  TestTLSInputStream *stream = TEST_TLS_INPUT_STREAM (object);
...
static void
test_tls_input_stream_constructed (GObject *object)
{
  TestTLSInputStream *stream = TEST_TLS_INPUT_STREAM (object);
...
static void
test_tls_input_stream_finalize (GObject *object)
{
  TestTLSInputStream *stream = TEST_TLS_INPUT_STREAM (object);

  g_object_unref (stream->session);

  G_OBJECT_CLASS (test_tls_input_stream_parent_class)
    ->finalize (object);
}

static void
test_tls_input_stream_class_init (GInputStreamClass *class)
{
  GObjectClass *obj_class = G_OBJECT_CLASS (class);

  class->read_fn = test_tls_input_stream_read;
  class->read_async = test_tls_input_stream_read_async;
  class->read_finish = test_tls_input_stream_read_finish;
  obj_class->set_property = test_tls_input_stream_set_property;
  obj_class->constructed = test_tls_input_stream_constructed;
  obj_class->finalize = test_tls_input_stream_finalize;
...
static void
test_tls_connection_init (TestTLSConnection *connection)
{
}
...
test_tls_session_read_ready (GObject      *object,
                             ...
                             gpointer      user_data)
{
  TestTLSSession *session = TEST_TLS_SESSION (user_data);
  ...
  if (rsize > 0)
    {
      int x;
      int y;
      S_DBG ("received %ld cipherbytes, filling SSL BIO", rsize);
   ...
   * or a fatal error (-ve rval)                                     */
    ...
    {
      S_DBG ("read of SSL cipherbytes failed: %ld", rsize);
      ...
      if ((*error != NULL) && ((*error)->domain == g_io_error_quark ()))
        {
          switch ((*error)->code)
            ...
            case G_IO_ERROR_PENDING:
              ...
              S_DBG ("retry op: [%d] %s", (*error)->code, (*error)->message);
              g_error_free (*error);
              *error = NULL;
              break;
            default:
              S_DBG ("failed op: [%d] %s", (*error)->code, (*error)->message);
              break;
            }
        }
       ...
       * we need to make sure the error in the job is set             */
        ...
        {
          *error =
            g_error_new (TEST_TLS_ERROR, SSL_ERROR_SSL, "unknown error");
        }
    }
                              ...
                              gpointer      user_data)
{
  ...
  written = g_output_stream_write_finish (G_OUTPUT_STREAM (object), result,
                                          &(session->job.write.error));
  g_output_stream_flush (G_OUTPUT_STREAM (object), NULL, NULL);

  if (written + session->extra_write == buffered)
    {
      gint ignore_warning;
      S_DBG ("%d bytes written, clearing write BIO", buffered);
  ...
  else
    {
      ...
      gint ignore_warning;
      S_DBG ("Incomplete async write: attempting to recover.");
...
static void
test_tls_session_init (TestTLSSession *session)
{
  const char *level;
  guint lvl = 0;
  ...
  if G_UNLIKELY (g_once_init_enter (&initialised))
                  {
                    S_DBG ("initialising SSL library and error strings");
                    CRYPTO_malloc_init ();
                    SSL_library_init ();
  ...
  if ((level = getenv ("TEST_TLS_DEBUG_LEVEL")) != NULL)
    lvl = atoi (level);
...
static void
test_tls_session_set_property (GObject *object, guint prop_id,
                               const GValue *value, GParamSpec *pspec)
{
  TestTLSSession *session = TEST_TLS_SESSION (object);
...
static void
set_dh_parameters (TestTLSSession *session)
{
  DH *dh;
  ...
  switch (session->dh_bits)
    ...
    case 512:
      ...
      break;
    default:
      S_DBG ("Bad dh-bits setting: %d, reset to 1024", session->dh_bits);
      dh = get_dh1024 ();
    }
...
static void
set_ecdh_key (TestTLSSession *session)
{
  EC_KEY *ecdh = EC_KEY_new_by_curve_name (NID_sect163r2);
  if (ecdh == NULL)
    {
      S_DBG ("unable to create elliptical crypto key for sect163r2 curve");
      return;
    }
...
static void
test_tls_session_constructed (GObject *object)
{
  TestTLSSession *session = TEST_TLS_SESSION (object);

  if (session->server)
    {
      S_DBG ("SSLv23_server_method");
      session->method = SSLv23_server_method ();
    }
  else
    {
      S_DBG ("SSLv23_client_method");
  ...
  if ((session->key_file != NULL) && (session->cert_file != NULL))
    {
      long errnum;
      S_DBG ("cert: %s", session->cert_file);
      S_DBG ("key : %s", session->key_file);
      ...
      if (!SSL_CTX_check_private_key (session->ctx))
        {
          ...
          S_DBG ("cert/key check: %ld %s", errnum, error_to_string (errnum));
        }
      else
        S_DBG ("certificate loaded");
    }
  ...
  if (session->rbio == NULL)
    S_DBG ("Could not allocate memory BIO for SSL reads");

  if (session->wbio == NULL)
    S_DBG ("Could not allocate memory BIO for SSL writes");
...
static void
test_tls_session_finalize (GObject *object)
{
  TestTLSSession *session = TEST_TLS_SESSION (object);
  ...
  G_OBJECT_CLASS (test_tls_session_parent_class)->finalize (object);
}

static void
test_tls_session_dispose (GObject *object)
{
  TestTLSSession *session = TEST_TLS_SESSION (object);
  ...
  G_OBJECT_CLASS (test_tls_session_parent_class)->dispose (object);
}

static void
test_tls_session_class_init (GObjectClass *class)
{
  class->set_property = test_tls_session_set_property;
  class->constructed = test_tls_session_constructed;
  class->finalize = test_tls_session_finalize;
  class->dispose = test_tls_session_dispose;
...
static void
test_tls_connection_set_property (GObject *object, guint prop_id,
                                  const GValue *value, GParamSpec *pspec)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (object);
...
static gboolean
test_tls_connection_close (GIOStream *stream, GCancellable *cancellable,
                           GError **error)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (stream);

  return g_io_stream_close (connection->session->stream, cancellable, error);
}

static GInputStream *
test_tls_connection_get_input_stream (GIOStream *io_stream)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (io_stream);

  if (connection->input == NULL)
    connection->input = g_object_new (TEST_TYPE_TLS_INPUT_STREAM,
...
test_tls_connection_get_output_stream (GIOStream *io_stream)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (io_stream);

  if (connection->output == NULL)
    connection->output = g_object_new (TEST_TYPE_TLS_OUTPUT_STREAM,
                                       "session", connection->session,
                                       NULL);
...
static void
test_tls_connection_get_property (GObject *object, guint prop_id,
...
static void
test_tls_connection_constructed (GObject *object)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (object);
...
static void
test_tls_connection_finalize (GObject *object)
{
  TestTLSConnection *connection = TEST_TLS_CONNECTION (object);
  ...
  G_OBJECT_CLASS (test_tls_connection_parent_class)
    ->finalize (object);
}

static void
test_tls_connection_class_init (TestTLSConnectionClass *class)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);
  GIOStreamClass *stream_class = G_IO_STREAM_CLASS (class);

  gobject_class->get_property = test_tls_connection_get_property;
  gobject_class->set_property = test_tls_connection_set_property;
  gobject_class->constructed = test_tls_connection_constructed;
  gobject_class->finalize = test_tls_connection_finalize;
  ...
  stream_class->get_input_stream = test_tls_connection_get_input_stream;
  stream_class->get_output_stream = test_tls_connection_get_output_stream;
  stream_class->close_fn = test_tls_connection_close;
}

TestTLSSession *
test_tls_session_new (GIOStream *stream)
{
  return g_object_new (TEST_TYPE_TLS_SESSION,
                       "base-stream", stream,
                       "server", FALSE, NULL);
}

/**
 * test_tls_session_server_new:
 ...
 *
 * Returns: a #TestTLSSession object
 */
TestTLSSession *
test_tls_session_server_new (GIOStream *stream, guint dhbits,
                             const gchar* key, const gchar* cert)
{
  if (dhbits == 0)
    dhbits = 1024;
  return g_object_new (TEST_TYPE_TLS_SESSION, "base-stream", stream,
