...
stream_opened(WockyXmppConnection *connection,
  ...
  WockyXmppNode *mechnode;

  fprintf(stderr, "STREAM OPENED to=%s from=%s version=%s\n",
           to, from, version);
  ...
  if (priv->mech != NULL) {
    wocky_xmpp_node_add_child_with_content(mechnode, "mechanism", priv->mech);
  } else {
    const gchar *mechs;
    gchar **mechlist;
    gchar **tmp;
    ...
    for (tmp = mechlist; *tmp != NULL; tmp++) {
      wocky_xmpp_node_add_child_with_content(mechnode, "mechanism", *tmp);
    }
    g_strfreev(mechlist);
  }
...
handle_auth(TestSaslAuthServer *self, WockyXmppStanza *stanza) {
  TestSaslAuthServerPrivate *priv = TEST_SASL_AUTH_SERVER_GET_PRIVATE(self);
  const gchar *mech = wocky_xmpp_node_get_attribute(stanza->node, "mechanism");
...
static void
received_stanza(WockyXmppConnection *connection,
                WockyXmppStanza *stanza,
                gpointer user_data) {
  TestSaslAuthServer *self = TEST_SASL_AUTH_SERVER(user_data);
  int i;
